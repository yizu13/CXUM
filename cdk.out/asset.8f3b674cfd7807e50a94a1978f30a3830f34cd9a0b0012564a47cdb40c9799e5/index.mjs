...
export const handler = async (event) => {
  try {
    ...
    await logActivity({ type: "usuario", icon: "solar:user-check-rounded-bold-duotone", color: "#22c55e", text: `Usuario actualizado: ${username}${role ? ` → ${role}` : ""}${status ? ` (${status})` : ""}`, actor: event.requestContext?.authorizer?.lambda?.username ?? "" });
