...
function validateResponse(form, rawValues) {
  ...
  for (const field of visibleFields) {
    ...
    if (field.required && (empty || raw === false)) {
...
async function listForms({ admin }) {
  const result = await ddb.send(new ScanCommand({ TableName: FORMS_TABLE }));
  let forms = result.Items ?? [];
...
async function listResponses(event) {
  ...
  if (formId) {
    ...
    const result = await ddb.send(new ScanCommand({ TableName: RESPONSES_TABLE }));
    items = result.Items ?? [];
  }
  ...
  try {
    ...
    if (method === "POST" && routePath.includes("/responses")) {
      ...
      const response = {
        ...
        locationLabel: String(body.locationLabel ?? "No especificado").slice(0, 120),
        values: validation.values,
        userAgent: body.userAgent ? String(body.userAgent).slice(0, 240) : "",
      };
