...
const INDEX_TABLE = process.env.CERTIFICATES_INDEX_TABLE;
const DESIGNS_TABLE = process.env.CERTIFICATE_DESIGNS_TABLE;
const MAX_TEMPLATE_SIZE = 40 * 1024 * 1024;
const MAX_GENERATION_FILE_SIZE = 250 * 1024 * 1024;
const MAX_BATCH_FILES = 1000;
const MAX_DESIGN_AREAS = 200;
...
export const handler = async (event) => {
  try {
    ...
    if (method === "GET" && path.includes("/certificates/") && !path.includes("/admin/")) {
      return getPublicCertificate(event);
    }
    ...
    if (method === "GET" && path.includes("/designs/")) {
      return getDesignFlow(event);
    }

    if (method === "PUT" && path.includes("/designs/")) {
      return saveDesignFlow(event, user);
    }

    if (method === "DELETE" && path.includes("/designs/")) {
      return deleteDesignFlow(event);
    }
...
async function deleteTemplate(event) {
  ...
  if (templateId) await deleteDesignByTemplateId(templateId);
  return ok({ message: "Plantilla eliminada", templateId, bucketPrefix: prefix });
}

async function getDesignFlow(event) {
  const templateId = event.pathParameters?.templateId;
  if (!isValidId(templateId)) return badRequest("templateId invalido");
  ...
  if (!result.Item) return notFound("Diseno de plantilla no encontrado");
  return ok({ design: result.Item });
}

async function saveDesignFlow(event, user) {
  const templateId = event.pathParameters?.templateId;
  const body = parseBody(event);
  if (!isValidId(templateId)) return badRequest("templateId invalido");

  const templateName = String(body.templateName ?? "").trim();
  const templateFileName = String(body.templateFileName ?? "").trim();
  const areas = Array.isArray(body.areas) ? body.areas : null;
  if (!templateName) return badRequest("templateName es requerido");
  if (!areas) return badRequest("areas debe ser un arreglo");
  if (areas.length > MAX_DESIGN_AREAS) return badRequest("El diseno contiene demasiadas areas");
  ...
  const design = {
    id: current.Item?.id ?? randomUUID(),
    templateId,
    templateName,
    templateFileName,
    areas,
    createdAt: current.Item?.createdAt ?? now,
    createdBy: current.Item?.createdBy ?? user.username,
    updatedAt: now,
...
async function deleteDesignFlow(event) {
  const templateId = event.pathParameters?.templateId;
  if (!isValidId(templateId)) return badRequest("templateId invalido");
  await deleteDesignByTemplateId(templateId);
  return ok({ message: "Diseno de plantilla eliminado", templateId });
}
...
async function completeGeneration(event) {
  ...
  const updated = {
    ...
    hasDigitalCertificates: Array.isArray(body.certificates) && body.certificates.length > 0,
    digitalCount: Array.isArray(body.certificates) ? body.certificates.length : current.digitalCount ?? 0,
  ...
  if (Array.isArray(body.certificates) && body.certificates.length > 0) {
    await Promise.all(body.certificates.map((certificate) => putCertificateIndex({
      certificateId: certificate.certificateId,
      key: certificate.key,
      fileName: certificate.fileName ?? certificate.key?.split("/").pop() ?? "",
      generationId: updated.id,
      templateId: updated.templateId,
      templateName: updated.templateName,
      bucketPrefix: updated.bucketPrefix,
...
async function deleteGeneration(event) {
  ...
  if (!prefix || !isSafePrefix(prefix, "generations/")) return badRequest("bucketPrefix o generationId invalido");
  if (generationId) await deleteCertificateIndexByGeneration(generationId);
  await deletePrefix(prefix.endsWith("/") ? prefix : `${prefix}/`);
  return ok({ message: "Generacion eliminada", generationId, bucketPrefix: prefix });
}

async function getPublicCertificate(event) {
  const certificateId = event.pathParameters?.certificateId;
  if (!certificateId || certificateId.includes("..")) return badRequest("ID de certificado invalido");
  ...
  if (!result.Item || result.Item.deletedAt) return notFound("Certificado no encontrado");
  ...
  const downloadUrl = await getSignedUrl(
    ...
    new GetObjectCommand({
      ...
      ResponseContentDisposition: `inline; filename="${result.Item.fileName || certificateId}.pdf"`,
    }),
    { expiresIn: 900 },
  );
...
async function putCertificateIndex(item) {
  if (!item.certificateId || !isSafeObjectKey(item.key)) return;
  await ddb.send(new PutCommand({
    TableName: INDEX_TABLE,
    Item: {
      ...item,
...
async function deleteCertificateIndexByGeneration(generationId) {
  if (!generationId) return;
  const result = await ddb.send(new QueryCommand({
    TableName: INDEX_TABLE,
    IndexName: "generationId-index",
    KeyConditionExpression: "generationId = :generationId",
    ExpressionAttributeValues: { ":generationId": generationId },
  }));

  await Promise.all((result.Items ?? []).map((item) => ddb.send(new DeleteCommand({
    TableName: INDEX_TABLE,
    Key: { certificateId: item.certificateId },
  }))));
}

async function deleteDesignByTemplateId(templateId) {
