...
export const handler = async (event) => {
  ...
  try {
    await ddb.send(new UpdateCommand({
      TableName: TABLE,
      Key: { token },
      UpdateExpression: "SET #status = :reserved, reservedBy = :email, reservedAt = :reservedAt, reservedUntil = expiresAtEpoch",
      ConditionExpression: [
        "attribute_exists(#token)",
        "expiresAtEpoch > :nowEpoch",
        "(#status = :active OR (#status = :reserved AND reservedBy = :email))",
      ].join(" AND "),
