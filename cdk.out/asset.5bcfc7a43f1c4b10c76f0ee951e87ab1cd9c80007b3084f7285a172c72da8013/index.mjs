...
const SELECT_MODES = new Set(["single", "multiple"]);
...
function normalizeField(field, index) {
  ...
  if (type === "select") {
    ...
    normalized.selectionMode = SELECT_MODES.has(field.selectionMode) ? field.selectionMode : "single";
    if (normalized.selectionMode === "multiple" && field.createRecordPerSelection) {
      normalized.createRecordPerSelection = true;
    }
...
function normalizeForm(body, existing = undefined, actor = "") {
  ...
  const splitRecordFields = fields.filter((field) => field.createRecordPerSelection);
  if (splitRecordFields.length > 1) {
    return { error: "Solo un campo puede crear un registro por seleccion" };
  }
  ...
  const respondentFieldId = body.respondentFieldId || (primaryCanIdentify ? body.primaryFieldId : undefined) || undefined;
  const respondentField = fields.find((field) => field.id === respondentFieldId);
  if (respondentFieldId && respondentField?.type === "select" && respondentField.selectionMode === "multiple") {
    return { error: "El campo identificador debe usar seleccion unica" };
  }
  ...
  const item = {
    ...
    allowRepeatSubmissions: Boolean(body.allowRepeatSubmissions),
    fields,
...
function formatResponseValue(value, fallback = "No especificado") {
...
function passesCondition(field, values) {
  ...
  const actualItems = valueItems(actual).map((item) => item.toLowerCase());
...
function validateResponse(form, rawValues) {
  ...
  for (const field of visibleFields) {
    ...
    if (field.type === "select" && field.selectionMode === "multiple") {
      if (!Array.isArray(raw)) {
        errors.push(`${field.label} debe contener una lista de opciones`);
        continue;
      }
      const selectedOptions = [...new Set(raw.map((item) => String(item).trim()).filter(Boolean))];
      if (selectedOptions.length > 50 || selectedOptions.some((option) => !(field.options ?? []).includes(option))) {
        errors.push(`${field.label} contiene una opcion no valida`);
      }
      values[field.id] = selectedOptions;
      continue;
    }

    if (field.type === "select" && Array.isArray(raw)) {
      errors.push(`${field.label} permite seleccionar una sola opcion`);
      continue;
    }
...
function identifierCounterKey(formId, fieldId, identifier) {
  const hash = createHash("sha256").update(`${formId}\0${fieldId}\0${identifier}`).digest("hex");
...
async function countHistoricalIdentifierResponses(form, field, identifier) {
  const responses = await queryAll({
    ...
    ProjectionExpression: "id, submissionGroupId, #values",
    ExpressionAttributeNames: { "#values": "values" },
  });
  return new Set(
    responses
      .filter((response) => normalizeIdentifierValue(field, response.values?.[field.id]) === identifier)
      .map((response) => response.submissionGroupId || response.id),
  ).size;
}

async function putResponses(responses) {
...
async function storeResponseWithIdentifierCounter(form, responses) {
  ...
  try {
    ...
    if (method === "POST" && routePath.includes("/responses")) {
      ...
      const respondentLabel = respondentFieldId
        ? formatResponseValue(validation.values[respondentFieldId])
        : "No especificado";
      const locationLabel = form.locationFieldId
        ? formatResponseValue(validation.values[form.locationFieldId])
        : "No especificado";
      ...
      return created({ ...responseRecords[0], recordsCreated: responseRecords.length });
    }
