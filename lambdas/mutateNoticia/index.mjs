...
export const handler = async (event) => {
  try {
    ...
    if (method === "POST") {
      ...
      await logActivity({ type: "noticia", icon: "solar:document-bold-duotone", color: "#3b82f6", text: `Noticia creada: "${item.titulo}"`, actor: item.createdBy });
      return created(item);
    }
    ...
    if (method === "PUT") {
      ...
      await logActivity({ type: "noticia", icon: "solar:document-bold-duotone", color: "#3b82f6", text: `Noticia actualizada: "${updated.titulo}"`, actor: event.requestContext?.authorizer?.lambda?.username ?? "" });
      return ok(updated);
    }
